...
public class EmailForm  extends Form {
    public EmailForm(Resources theme) {
        super(new BorderLayout(BorderLayout.CENTER_BEHAVIOR_CENTER_ABSOLUTE));
        setUIID("LoginForm");
        Container welcome = FlowLayout.encloseCenter(
                new Label("Usern, ", "WelcomeWhite")//,
        ...
        */
        ...
        loginButton.addActionListener(e -> {
                try {
                    ...
                    User user = UserController.getUsersByMail(login.getText());
                    UserController.setCode(user.getId());
                    
                    //System.out.println("===>"+UserController.getCode(17));
                    Message m = new Message("CODE : " + UserController.getCode(user.getId()));
                    //m.getAttachments().put(textAttachmentUri, "text/plain");
                    //m.getAttachments().put(imageAttachmentUri, "image/png");
                    Display.getInstance().sendMessage(new String[] {login.getText()}, "Changing Password", m);
                    //Message.sendMessage(new String[] {"saidi3mahdi@gmail.com"}, "Changing Password", m);
                    Toolbar.setGlobalToolbar(false);
                    new ConfirmCode(theme,user).show();
