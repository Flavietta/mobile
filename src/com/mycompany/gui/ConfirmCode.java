 ...
 * @author FALAVIO
 */
public class ConfirmCode  extends Form {
    public ConfirmCode(Resources theme , User user) {
        super(new BorderLayout(BorderLayout.CENTER_BEHAVIOR_CENTER_ABSOLUTE));
        setUIID("LoginForm");
        Container welcome = FlowLayout.encloseCenter(
                new Label("Confirm Code, ", "WelcomeWhite")//,
        ...
        */
        TextField login = new TextField("", "Code", 20, TextField.EMAILADDR) ;
        login.getAllStyles().setMargin(LEFT, 0);
        Label loginIcon = new Label("", "TextField");
        loginIcon.getAllStyles().setMargin(RIGHT, 0);
        FontImage.setMaterialIcon(loginIcon, FontImage.MATERIAL_CONTACT_PHONE, 3);
        
        Button loginButton = new Button("Check");
        ...
        loginButton.addActionListener(e -> {
                try {
                    ...
                    if (UserController.getCode(user.getId())==Integer.valueOf(login.getText())) {
                        Toolbar.setGlobalToolbar(false);
                        new ChangePassword(theme , user).show();
